# Game Development on pulse2d

Below are notes on building games with pulse2d on the Teensy 4.1 — covering the memory layout,
...
doing nothing. `shift_game.cc` registers an aborting handler at the top of
...
```
shift_game.cc:42:5  main  128  static
```
...
static-init time, before `setup()` runs. Use `pulse2d::HARDWARE_Deferred_Init<T>`:

```cpp
static pulse2d::HARDWARE_Deferred_Init<pulse2d::Pulse2d> engine;

void setup() {
    engine.emplace();   // constructs Pulse2d here, hardware is ready
    engine->init();
}
```
...
1. **Large static arrays** — sprite pixel data, tile maps, audio buffers. Move them
   to OCRAM with `PULSE2D_EXTMEM` (defined in `pulse2d/display.h`):
   ```cpp
...
code, just like `.bss`. You do not need to zero `PULSE2D_EXTMEM` variables yourself.
...
```python
# rough conversion — see shift/images/ for reference
...
Each `pulse2d::Sprite` struct holds a pointer to pixel data plus width and height.
...
```cpp
static pulse2d::graphics::World world({ 0.0f, -10.0f }, 16);  // max 16 bodies
```
...
```bash
arm-none-eabi-nm build-teensy/shift_game.elf | sort | grep -v " [tT] "
```

Symbols at `0x20000000–0x2007ffff` are in DTCM. Symbols at `0x20200000+` are in
OCRAM. If something large is in DTCM that should not be, add `PULSE2D_EXTMEM`.

**Section breakdown:**

```bash
arm-none-eabi-objdump -h build-teensy/shift_game.elf | \
...
| Situation | Solution |
|-----------|----------|
| Large static buffer eating DTCM | `static PULSE2D_EXTMEM type name[N]` |
