...
make -f Makefile.teensy asm       # compile game sources to ARM assembly listings
...
The parallax background system blits full-screen sprites in layers each frame, scrolling each layer at its own speed. Sprites are loaded from C headers (see [`png2header`](#tools)) rather than the SD card, so they are available without any file I/O.
...
- **`PULSE2D_ADD_SCROLLING_LAYER(sprite_name, width, speed)`** — adds a parallax scroll layer to the current scene. `width` is the image width in pixels (used to wrap the offset); `speed` is pixels per second. Layers are drawn in the order they are added. Call after the corresponding `PULSE2D_SET_SPRITE_FLASH`.
  ```cpp
  PULSE2D_ADD_SCROLLING_LAYER(sprite_nebula, 320.0f, 10.0f);   // slow background
  PULSE2D_ADD_SCROLLING_LAYER(sprite_stars,  320.0f,  3.0f);   // very slow
  PULSE2D_ADD_SCROLLING_LAYER(sprite_dust,   320.0f, 65.0f);   // fast foreground
  ```
